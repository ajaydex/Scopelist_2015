...
Partial Class BVModules_CategoryTemplates_Scopelist_4COL_InStock_Category
    ...
    Protected Sub PageLoad(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If LocalCategory IsNot Nothing Then
            ...
            Select Case LocalCategory.DisplaySortOrder
                Case 1
                    displayProducts = allDisplayProducts.Where(Function(p) p.IsInStock = True).OrderByDescending(Function(p) p.IsInStock).ThenByDescending(Function(p) p.CreationDate).ToList()
                    rowCount = displayProducts.Count

                    displayProducts = displayProducts.Skip(Pager1.CurrentRow).Take(Pager1.ItemsPerPage).ToList()
                Case Else
