...
Partial Class BVModules_CategoryTemplates_Scopelist_4COL_All_Demo_InStock_Category
    ...
    Protected Sub PageLoad(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If LocalCategory IsNot Nothing Then
            ...
            Select Case LocalCategory.DisplaySortOrder
                Case 1
                    Dim DemoBackOrder As List(Of Catalog.Product)
                    DemoBackOrder = allDisplayProducts.Where(Function(p) p.ProductName.ToLower().Contains("demo") And p.IsInStock = False).ToList()

                    Dim UsedBackOrder As List(Of Catalog.Product)
                    UsedBackOrder = allDisplayProducts.Where(Function(p) p.ProductName.ToLower().Contains("used") And p.IsInStock = False).ToList()

                    Dim BlemishBackOrder As List(Of Catalog.Product)
                    BlemishBackOrder = allDisplayProducts.Where(Function(p) p.ProductName.ToLower().Contains("blemish") And p.IsInStock = False).ToList()

                    Dim AllRegular As New List(Of Catalog.Product)

                    For Each p As Catalog.Product In allDisplayProducts
                        If Not DemoBackOrder.Contains(p) And Not UsedBackOrder.Contains(p) And Not BlemishBackOrder.Contains(p) Then
                            AllRegular.Add(p)
                        Else
                            rowCount = rowCount - 1
                        End If
                    ...
                    displayProducts = AllRegular.OrderByDescending(Function(p) p.IsInStock).ThenByDescending(Function(p) p.CreationDate).Skip(Pager1.CurrentRow).Take(Pager1.ItemsPerPage).ToList()
                Case Else
    ...
    Protected Sub DataList1_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.DataListItemEventArgs) Handles DataList1.ItemDataBound
        If e.Item.ItemType = ListItemType.Item Or e.Item.ItemType = ListItemType.AlternatingItem Then
            ...
            If p IsNot Nothing Then
                ...
                anchor.InnerHtml = "<img src=""" & ResolveUrl(imageUrl) & """ width=""181"" height=""106"" />" & "<p style='text-align:center;padding-bottom: 0px;'>" & p.ProductName & "</p>"
