...
Partial Class BVModules_Controls_Pager
    ...
    Public Sub InitializeButtons()
        ...
        If Not String.IsNullOrEmpty(currentUrl) Then
            ...
            If Me.CurrentPage = 1 Then
                If Not FirstListItem.InnerText.Contains("First") Then
                    FirstListItem.InnerText &= Content.SiteTerms.GetTerm("First")
                End If
                If Not PreviousListItem.InnerText.Contains("Pre") Then
                    PreviousListItem.InnerText &= Content.SiteTerms.GetTerm("Previous")
                End If
                Me.FirstListItem.Attributes.Add("class", "disabled")
                Me.PreviousListItem.Attributes.Add("class", "disabled")
            Else
                Dim FirstHyperLink As New HyperLink()
                FirstHyperLink.Text = "&laquo;" & Content.SiteTerms.GetTerm("First")
                FirstHyperLink.NavigateUrl = currentUrl & urlSeperator & "1"
                FirstListItem.Controls.Clear()
                FirstListItem.Controls.Add(FirstHyperLink)

                Dim PreviousHyperLink As New HyperLink()
                PreviousHyperLink.Text = "&laquo;" & Content.SiteTerms.GetTerm("Previous")
                PreviousHyperLink.NavigateUrl = currentUrl & urlSeperator & (Me.CurrentPage - 1).ToString()
                PreviousListItem.Controls.Clear()
                PreviousListItem.Controls.Add(PreviousHyperLink)
            ...
            If Me.CurrentPage = Me.GetPageCount Then
                If Not LastListItem.InnerText.Contains("Last") Then
                    LastListItem.InnerText = Content.SiteTerms.GetTerm("Last") & ltlRightArrow.Text
                End If
                If Not NextListItem.InnerText.Contains("Next") Then
                    NextListItem.InnerText = Content.SiteTerms.GetTerm("Next") & ltlRightArrow.Text
                End If
            ...
            Else
                ...
                LastHyperLink.Text = Content.SiteTerms.GetTerm("Last") & "&raquo;"
                ...
                NextHyperLink.Text = Content.SiteTerms.GetTerm("Next") & "&raquo;"
            ...
            For i As Integer = 1 To 11
                If (startPageNumber + (i - 1)) <= Me.GetPageCount Then
                    'if start page number is equal to getpagecount and i value equal to 1 then reset start page number to GetPageCount-1 and GetPageCount != 1
                    If startPageNumber = Me.GetPageCount And i = 1 And Me.GetPageCount <> 1 Then
                        startPageNumber = Me.GetPageCount - 1
                    End If
