...
Partial Class BVModules_ProductTemplates_Bvc2013_Product
    ...
    Public Overrides Sub PopulateProductInfo(ByVal IsValidCombination As Boolean)
        ...
        If IsValidCombination Then
            ...
            Me.trSalePrice.Visible = False

            ' list price
            If LocalProduct.ListPrice > LocalProduct.SitePrice AndAlso productDisplay.ListPriceGreaterThanCurrentPrice Then
            ...
            Me.lblSitePrice.CssClass = "SitePrice"

            If Not productDisplay.HasPriceOverrideText Then
                If LocalProduct.SitePrice > productDisplay.SitePrice Then
                    ' site price (w/o sale)
                    Me.lblSitePrice.Text = String.Format("{0:c}", LocalProduct.SitePrice)
                    Me.lblSitePrice.CssClass = "SitePrice discount"

                    ' sale price
                    Me.trSalePrice.Visible = True
                    Me.lblSalePrice.Text = productDisplay.SitePriceDisplay
                End If

                ' savings
                If Me.trListPrice.Visible Then
                    ' savings (list price - site price w/sale)
                    Me.trYouSave.Visible = True
                    Me.lblYouSave.Text = productDisplay.Savings.ToString("c")
                ElseIf LocalProduct.SitePrice > productDisplay.SitePrice Then
                    ' savings (site price - site price w/sale)
                    Me.trYouSave.Visible = True
                    Me.lblYouSave.Text = Math.Round((LocalProduct.SitePrice - productDisplay.SitePrice), 2).ToString("c")
                End If
            End If
