<%@ Control Language="C#" AutoEventWireup="false" CodeFile="ShippingCostRules.ascx.cs" Inherits="BVModules_Shipping_Order_Rules_ShippingCostRules" %>
...
<%@ Register TagPrefix="uc" TagName="OrderMatchEditor" Src="OrderMatchEditor.ascx" %>
...
<div class="section">
    ...
    <p>
        The shipping cost rules are scanned in order until a match is found.
        The matching calculation is applied to the order to determine the
        shipping cost.
    </p>
    ...
    <asp:ObjectDataSource ID="RuleDataSource" runat="server" 
        ...
        DeleteMethod="Delete"
        SelectMethod="Select"
        TypeName="StructuredSolutions.Bvc5.Shipping.Providers.Settings.OrderRuleDataSource"
        UpdateMethod="Update">
        ...
        <UpdateParameters>
            ...
            <asp:Parameter Name="valueOrderProperty" Type="String" />
        ...
        <Columns>
            <asp:TemplateField HeaderStyle-HorizontalAlign="left">
                ...
                <ItemTemplate>
                	<div style="padding: 20px;">
                    	<div style="margin-bottom: 15px;">
                            <%-- New --%>
                            <asp:ImageButton ID="InsertRow" runat="server" CommandName="New" CommandArgument='<%# Eval("ID") %>' ImageUrl="~/BVAdmin/Images/Buttons/New.png" ToolTip="Insert a new rule before this rule" />
                            <%-- Edit --%>
                            <asp:ImageButton ID="EditRow" runat="server" CommandName="Edit" ImageUrl="~/BVAdmin/Images/Buttons/Edit.png" ToolTip="Edit this rule" />
                            <%-- Delete --%>
                            <anthem:ImageButton ID="DeleteRow" runat="server" CommandName="Delete" ImageUrl="~/BVAdmin/Images/Buttons/Delete.png" PreCallBackFunction="function(){return confirm('Are you sure you want to delete this rule?');}" ToolTip="Delete this rule" />
                    	</div>
                        
                        <ol style="list-style: none; margin: 0; padding: 0;">
                            <li style="margin: 0; padding: 0;" value="<%# (Rules.PageIndex * Rules.PageSize) + ((GridViewRow)Container).RowIndex + 1%>">
                ...
                <EditItemTemplate>
                	<div style="padding: 20px;">
                        <%-- Cancel --%>
                        <asp:ImageButton ID="CancelEdit" runat="server" CausesValidation="false" CommandName="Cancel" ImageUrl="~/BVAdmin/Images/Buttons/CancelSmall.png" ToolTip="Quit editing this rule without saving changes"></asp:ImageButton>

                        <asp:ValidationSummary ID="RuleGroupValidationSummary" runat="server" DisplayMode="SingleParagraph" EnableClientScript="false" ValidationGroup="RuleGroup"></asp:ValidationSummary>

                        <table class="formtable subform">
                            <tr id="tr1" runat="server" visible='<%# !Settings.CostingRules[Eval("ID").ToString()].IsDefaultRule %>'>
                                <td>
                                    <h2>Match orders that satisfy all of the following conditions</h2>
                                </td>
                            </tr>
                            <tr>
                                <td>
                                    <uc:OrderMatchEditor ID="OrderMatchEditor" runat="server" DataSource='<%# Eval("Matches") %>'  RuleId='<%# Eval("ID") %>' />
                                </td>
                            </tr>
                            <tr>
                                <td>
                                    <h2><%# Settings.CostingRules[Eval("ID").ToString()].IsDefaultRule ? "For all remaining orders, assign the following cost" : "Assign the following cost" %></h2>
                                </td>
                            </tr>
                            <tr>
                                <td>
                                    Cost = <ss:HelpLabel ID="ValueOrderPropertyLabel" runat="server"
                                        AssociatedControlID="ValueOrderPropertyField"
                                        Text="Value Property"
                                        ToolTip="&lt;p&gt;Select the order property that will be used to 
                                            ...
                                            value if the value is a number.&lt;/p&gt;
                                            &lt;h2&gt;Items&lt;/h2&gt;
                                            ...
                                            &lt;p&gt;If the item property is not numeric, then the value
                                            will be treated as 0.&lt;/p&gt;
                                            &lt;h2&gt;Rate From&lt;/h2&gt;
                                            &lt;p&gt;If the selected shipping method returns more than one rate,
                                            the least expensive rate will be used in the comparison.&lt;/p&gt;"
                                        Visible="false">
                                    </ss:HelpLabel>
                                    <asp:DropDownList ID="ValueOrderPropertyField" runat="server" 
                                        AutoPostBack="true" 
                                        OnSelectedIndexChanged="ValueOrderPropertyField_SelectedIndexChanged" 
                                        SelectedValue='<%# Bind("ValueOrderProperty") %>'>
                                    </asp:DropDownList>
                                    <asp:RegularExpressionValidator id="ValuePropertyValidator" runat="server" 
                                        ControlToValidate="ValueOrderPropertyField"
                                        Display="Dynamic"
                                    ...
                                    <ss:HelpLabel ID="ValuePackagePropertyLabel" runat="server"
                                        AssociatedControlID="ValuePackagePropertyField"
                                        Text="Value Package Property"
                        ...
                        <asp:GridView id="SampleShippingCosts" runat="server"
                            ...
                            <Columns>
                                <asp:BoundField DataField="OrderDisplay" HeaderText="Order Number" HtmlEncode="false" />
                        ...
                        <%-- Update --%>
                        <asp:ImageButton ID="UpdateRule" runat="server" CommandName="Update" ImageUrl="~/BVAdmin/Images/Buttons/Update.png" ToolTip="Save the changes you have made to this rule" ValidationGroup="RuleGroup"></asp:ImageButton>
                        <%-- View --%>
                        <anthem:ImageButton ID="ViewRule" runat="server" CommandName="View" ImageUrl="~/BVAdmin/Images/Buttons/View.png" TextDuringCallBack="Working..." ToolTip="Display a sample of old orders that match this rule" ValidationGroup="RuleGroup"></anthem:ImageButton>
                    </div>
                </EditItemTemplate>
            ...
            <asp:TemplateField>
                ...
                <ItemTemplate>
                    <div>
                    	<asp:ImageButton ID="MoveRuleUp" runat="server" ImageUrl="~/BVAdmin/Images/Buttons/Up.png" CommandArgument='<%# Eval("ID") %>' CommandName="MoveUp" ToolTip="Move this rule up in order" />
                    </div>
                    <div>
                    	<asp:ImageButton ID="MoveRuleDown" runat="server" ImageUrl="~/BVAdmin/Images/Buttons/Down.png" CommandArgument='<%# Eval("ID") %>' 
CommandName="MoveDown" ToolTip="Move this rule down in order" />
                  	</div>
                </ItemTemplate>
            </asp:TemplateField>
        </Columns>
